...
Function update()
 For ca=0 To w/2-1
  For nn=angle(ca) To angle(ca)+360 Step 60
   ...
   xx=w/2+Cos((2*((angle(ca)+nn)*182))*Pi/360)*(ca+ca) ;add +nn next to the ca in (ca) at the end of
   yy=h/2+Sin((2*((angle(ca)+nn)*182))*Pi/360)*(ca+ca) ;these lines, for another effect
   Plot xx,yy
  Next
